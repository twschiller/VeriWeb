...
public class IntelliMapVisitor extends ASTVisitor{
	...
	public IntelliMapVisitor(IntelliMap map){
		...
		if (log == null){
			 try {
				log = new PrintWriter(new BufferedWriter(new FileWriter("/home/tws/veriweb-eclipse.log")));
	...
	public IntelliNode doMethod(MethodDeclaration method){
		if (method.getReturnType2() == null){
			...
			if (!map.isMapped(name)){
				IntelliNode in = new IntelliNode(name,"");
				map.addType(method.getReturnType2().resolveBinding().getQualifiedName(), in);
				return in;
			}else{
				return map.getIntelliNode(name);
			}
	...
	public boolean visit(TypeDeclaration node){
		...
		map.addType(node.getName().getFullyQualifiedName(), genNode(node));
		return true;
	}
